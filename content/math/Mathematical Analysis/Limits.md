...
is saying:
$$
\forall \epsilon > 0, \exists N >0, \forall n >N, |x_{n} - A| \leq \epsilon
$$
## Limits of function
...
$$
\forall \epsilon > 0, \exists x_{0} > 0, \forall x > x_{0}, |f(x) - A| < \epsilon
$$
...
> If a function $f$ has a limit $A$ on $a$ , then there's a punctured neighborhood $I_{a}$ of $a$, one that set, $f$ keeps the same sign with the limit $A$

Suppose $\lim_{ x \to a }f(x) = A > 0$
...
\implies & \forall \epsilon > 0, \exists\delta_{1}, \delta_{2} > 0, \delta = \min(\delta_{1}, \delta_{2}), \forall x \in (a-\delta, a) \cup (a, a + \delta), |g(x) - A| < \epsilon, |h(x) - A| < \epsilon \\
...
> If $f(x) \leq g(x)$, and $\lim_{ x \to a }f(x) = A, \lim_{ x \to a }g(x) = B$, then $A \leq B$

if $A > B$, then for $\epsilon=\frac{A-B}{2}$, $\exists \delta_{1}, \delta_{2} > 0, \delta=\min(\delta_{1},\delta_{2}), \forall x \in (a-\delta,a) \cup (a, a + \delta)$:
...
with $\delta_{1}$, $|f(x) - L| \leq \frac{\epsilon}{2}$
with $\delta_{2}$, $|g(x) - M| \leq \frac{\epsilon}{2}$
Thus
$$
\begin{align}
...
2. be aware that $f(x)$ is locally bounded due to the [[math/Mathematical Analysis/Limits#Local Boundness]]

$\forall \epsilon > 0$:
- with $\delta_{1}$, $|f(x) - L| < \frac{\epsilon}{2(|M| + 1)}$, thus $|f(x)| < |f(x) - L| + |L| < \frac{\epsilon}{2(|M| + 1)} + |L| = K$
- with $\delta_{2}, |g(x) - M| < \frac{\epsilon}{2K}$,

so, with $\delta = \min(\delta_{1}, \delta_{2})$, 
$$
\begin{align}
...
step1: Proof $\lim_{ x \to a } \frac{1}{g(x)} = \frac{1}{B}$
$$
\begin{align}
...
step2: use [[math/Mathematical Analysis/Limits#distributive to function product]]:

$$
\lim_{ x \to a } \frac{f(x)}{g(x)} = \lim_{ x \to a } f(x) \frac{1}{g(x)} = \frac{A}{B}
$$
...
1. proof $\lim_{ x \to a } \frac{1}{ g(x)} = \frac{1}{M}$ and reverse triangular inequality
2. product rule

with $\delta_{1}$, $|g(x) - M| \le \frac{|M|}{2}$, which saids: 
...
\end{align}
$$
with $\delta_{2}$, $|g(x) - M| \leq \frac{|M|^{2}\epsilon}{2}$
...
$$
\lim_{ x \to c } \frac{f(x)}{g(x)} = \lim_{ x \to c } f(x) \lim_{ x \to c } \frac{1}{g(x)} = \frac{L}{M}
$$
...
$$
\begin{align}
\forall x \in \dot{U}(x_{0}, \delta_{x}), u=g(x) \in U(u_{0}, \delta_{u}) \\
\forall u \in \dot{U}(u_{0}, \delta_{u}), y=f(x) \in U(y_{0}, \epsilon) \\
f(u_{0})=\underbrace{ y_{0} }_{ \text{ the limit} } \\
\implies \forall x \in \dot{U}(x_{0}, \delta_{x}), y=f(x) \in U(y_{0}, \epsilon)
\end{align}
$$
