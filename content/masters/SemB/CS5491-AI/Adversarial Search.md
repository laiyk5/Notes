...
So what max node is doing when it is calling is children is just like saying: "hey boy, I've asked your brothers and found that the biggest value is $\alpha\_{new}$, and my parent say he wants a score smaller than $\beta$, so go find it!". What's more, when the max node found $\alpha > \beta$, it knows no more legal value would be found, so it simply stops and returns.
